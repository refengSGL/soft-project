...
public class LendRecordController {
    ...
    public  Result<?> deleteRecord(@RequestBody LendRecord LendRecord){
        Map<String,Object> map = new HashMap<>();
        map.put("number",LendRecord.getNumber());
    ...
    public Result<?> findPage(@RequestParam(defaultValue = "1") Integer pageNum,
        ...
        if(StringUtils.isNotBlank(search1)){
            wrappers.like(LendRecord::getNumber,search1);
        }
        if(StringUtils.isNotBlank(search2)){
            wrappers.like(LendRecord::getHouseName,search2);
        }
        if(StringUtils.isNotBlank(search3)){
            wrappers.like(LendRecord::getUserId,search3);
        }
